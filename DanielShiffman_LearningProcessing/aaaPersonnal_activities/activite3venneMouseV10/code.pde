/*
 // calcul de arcAB et ws (arcAB/2) le long de C1, la demi longueur de l'arc de l'intersection C1/C3
 ...
 float demip = 0.5 * ((2 * R) + AB); // demi périmètre triangle OAB
 println("demip = " + demip);

 float sinO = (2/sq(R)) * sqrt(demip * sq(demip - R) * (demip - AB));
 println("sinO = " + sinO + " radians");

 float arcAB = asin(sinO);
 println("arcAB = " + arcAB + " radians");

 float ws = arcAB/2;
 println("ws = " + ws + " radians");
